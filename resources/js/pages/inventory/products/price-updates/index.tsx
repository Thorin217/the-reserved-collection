...
const PRODUCT_PRICE_UPDATES_PREVIEW_URL =
    '/admin/products/price-updates/preview';
const PRODUCT_PRICE_UPDATES_STORE_URL = '/admin/products/price-updates';
const PRODUCT_PRICE_UPDATES_HISTORY_URL =
    '/admin/products/price-updates/history';
...
export default function ProductPriceUpdatesIndex({
    attributes,
    filters,
    preview,
    execution,
    ...
    const attributesById = useMemo(
        () =>
            new Map(
                attributes.data.map((attribute) => [attribute.id, attribute]),
            ),
        [attributes.data],
    );
    const variantIds = useMemo(
        () => preview.variants.map((variant) => variant.id),
        [preview.variants],
    );

    const canSubmitPreview = selectedFilters.some(
        (filter) => filter.attribute_id !== null,
    );
    const parsedChangeValue = Number(changeValue);
    const isValidChangeValue =
        Number.isFinite(parsedChangeValue) && Math.abs(parsedChangeValue) > 0;
    const canApplyUpdate = selectedVariantIds.length > 0 && isValidChangeValue;
    const allSelected =
        variantIds.length > 0 &&
        selectedVariantIds.length === variantIds.length;
    const latestFluctuations = useMemo(
        () => recentFluctuations.slice(0, 3),
        [recentFluctuations],
    );
    ...
    function updateFilterAttribute(index: number, attributeId: string) {
        if (attributeId === EMPTY_VALUE) {
            setSelectedFilters((previous) =>
                previous.map((filter, currentIndex) => {
        ...
        setSelectedFilters((previous) =>
            previous.map((filter, currentIndex) => {
                ...
                return {
                    ...filter,
                    attribute_option_id:
                        optionId === EMPTY_VALUE ? null : Number(optionId),
                };
            }),
        );
    }
    ...
    function applyUpdate() {
        ...
        router.post(
            PRODUCT_PRICE_UPDATES_STORE_URL,
            {
                name: 'Bulk update',
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div>
                    ...
                    <Link
                        href={PRODUCT_PRICE_UPDATES_HISTORY_URL}
                ...
                <Card className="hidden lg:block">
                    ...
                    <CardContent className="space-y-2">
                        {latestFluctuations.length === 0 && (
                            <p className="text-sm text-muted-foreground">
                                No previous price fluctuations recorded yet.
                            </p>
                        )}
                                ...
                                >
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        <p className="font-medium">
                                            ...
                                            <span
                                                className={`text-sm ${Number(fluctuation.change_value) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                                            >
                                                (
                                                {Number(
                                                    fluctuation.change_value,
                                                ) > 0
                                                    ? '+'
                                                    : ''}
                                                {fluctuation.change_value}%)
                                        ...
                                        <Link
                                            href={`${PRODUCT_PRICE_UPDATES_HISTORY_URL}/${fluctuation.id}`}
                                            className="text-sm text-primary underline"
                                        >
                                            View detail
                                        </Link>
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        {fluctuation.affected_variants_count}{' '}
                                        variants •{' '}
                                        {fluctuation.affected_products_count}{' '}
                                        products • by{' '}
                                        {fluctuation.creator_name ?? 'System'} •{' '}
                                        {formatDate(fluctuation.created_at)}
                                    </p>

                                    {fluctuation.affected_products.length >
                                        0 && (
                                        <p className="mt-1 text-xs text-muted-foreground">
                                            Affected products:{' '}
                                            {fluctuation.affected_products.join(
                                                ', ',
                                            )}
                                            {fluctuation.affected_products_count >
                                                fluctuation.affected_products
                                                    .length && '...'}
                                        </p>
                                    )}
                                </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Filter by product-level attributes. Option is
                            optional.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={submitPreview} className="space-y-3">
                            {selectedFilters.map((filter, index) => {
                                const selectedAttribute = filter.attribute_id
                                    ? attributesById.get(filter.attribute_id)
                                    : null;
                                const options =
                                    selectedAttribute?.attribute_options ?? [];
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <SearchableSelect
                                            ...
                                            onValueChange={(value) =>
                                                updateFilterOption(index, value)
                                            }
                        ...
                        <div className="flex items-center justify-between border-b px-4 py-2 text-sm">
                            ...
                            <div className="flex gap-2">
                                <Button
                                    ...
                                    disabled={
                                        variantIds.length === 0 || allSelected
                                    }
                                >
                                    Select all
                                </Button>
                            ...
                            <TableBody>
                                ...
                                {preview.variants.length === 0 && (
                                    <TableRow>
                                        ...
                                        >
                                            No candidate variants yet. Add
                                            filters and run preview.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {!isValidChangeValue && (
                                <p className="text-xs text-muted-foreground">
                                    Enter a non-zero percentage value.
                                </p>
                            )}
                            ...
                            {execution?.applied && (
                                <p className="text-sm text-muted-foreground">
                                    Last execution applied{' '}
                                    {execution.change_value}% to{' '}
                                    {execution.affected_variants_count} variants
                                    successfully.
                                </p>
                            )}
                        </div>
