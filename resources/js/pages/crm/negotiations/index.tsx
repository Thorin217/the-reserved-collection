...
export default function NegotiationsIndex({ negotiations, users, filters }: Props) {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">{negotiations.meta.total} negotiations total</p>
                    </div>
                </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {negotiations.data.map((negotiation) => {
                                    const status = STATUS_CONFIG[negotiation.status] ?? STATUS_CONFIG.negotiating;
                                    return (
                                        <TableRow
                                            key={negotiation.id}
                                            className={negotiation.lead ? 'cursor-pointer' : undefined}
                                            onClick={() => negotiation.lead && router.visit(negotiationShow({ lead: negotiation.lead, negotiation }))}
                                        >
                                            <TableCell className="font-medium">
                                                {negotiation.lead ? (
                                                    <Link
                                                        href={leadShow(negotiation.lead)}
                                            ...
                                            <TableCell className="text-sm text-muted-foreground">
                                                {negotiation.lead?.client?.name ?? '—'}
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant={status.variant}>{status.label}</Badge>
                                            </TableCell>
                                            <TableCell className="text-right font-mono text-sm">
                                                {formatPrice(negotiation.initial_price)}
                                            </TableCell>
                                            <TableCell className="text-right font-mono text-sm">
                                            ...
                                            <TableCell className="text-sm text-muted-foreground">
                                                {negotiation.agreed_at
                                                    ? new Date(negotiation.agreed_at).toLocaleDateString()
                                                    : '—'}
                                            </TableCell>
                                                ...
                                                <div className="flex items-center justify-end">
                                                    {negotiation.lead && (
                                                        <Button variant="ghost" size="icon" asChild title="View negotiation">
                                                            <Link href={negotiationShow({ lead: negotiation.lead, negotiation })}>
...
NegotiationsIndex.layout = (page: React.ReactNode) => (
    <AppLayout breadcrumbs={[{ title: 'CRM', href: '#' }, { title: 'Negotiations', href: negotiationsIndex() }]}>
        {page}
    </AppLayout>
);
